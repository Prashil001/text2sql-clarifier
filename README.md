...
> *"Architected and built a production-grade Text-to-SQL system that eliminates LLM hallucinations and security vulnerabilities through dynamic schema grounding and AST-based SQL validation using sqlglot. Engineered a Human-in-the-Loop clarification engine that disambiguates under-specified questions before query generation. Deployed the complete stack (FastAPI, PostgreSQL, Streamlit) via Docker Compose, leveraging a hybrid host gateway for local GPU-accelerated Ollama inference."*
